...
def main(args):
    """
    Main training function for transformer-based malicious URL detection.
    Supports DomURLs_BERT and URLBert models for URL classification tasks only.
    """
    # Set random seed for reproducibility
    seed_everything(seed=args.seed)
    
    # Set dataset path - only URL datasets are supported
    path = f'data/url_datasets/{args.dataset}'
    
    # Set the max sequence length for URLs
    max_length = 128  # Optimized for URL classification
    ...
    "experiment_type": "url",  # Fixed to URL only
    ...
    # Create datasets for transformer models
    ...
    "URL_labels": ", ".join(label_encoder.classes_),
    "mlflow.note.content": f"Transformer-based deep learning for malicious URL detection using {args.pretrained_path} model",
    })

    mlflow.set_tracking_uri(Path.cwd().joinpath("mlruns/experiments").as_uri())
    exp_name = f'{args.dataset}_url_classification_{num_classes==2}'
    ...
    # Determine accelerator and precision based on device
    if args.device == -1:
        accelerator = "cpu"
        devices = "auto"
    ...
    trainer = Trainer(max_epochs=args.epochs, devices=devices, accelerator=accelerator, logger=mlf_logger, callbacks=[checkpoint_callback, early_stopping],  benchmark=False,
                  deterministic=True, precision=precision, gradient_clip_val=1)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train transformer-based models for malicious URL detection.")
    parser.add_argument('--dataset', type=str, default='Mendeley_AK_Singh_2020_phish', help='URL dataset name')
    parser.add_argument('--pretrained_path', type=str, default='amahdaouy/DomURLs_BERT', help='Pretrained transformer model path')
